...
| F | （侵攻中）勇者をカメラで追う / 解除 |
| F12 | スクリーンショット（`user://`） |
| 魔物をクリック（ブレーカー） | 最大HPの1/3のダメージ（元気な魔物は3回で倒れ、養分は死亡時と同じく周りの土にまき散らす。掘れる回数は減らない） |
...
| A | 掘る・決定（魔王の配置・メニュー・カットインのスキップ）。**押しながら十字キーで一直線に連続で掘る**。押している間ブレーカーは素振りを続ける |
| B | キャンセル（一時停止メニューを閉じる、「勇者を呼びますか？」で「いいえ」） |
| Y | （建設中）勇者を呼ぶ（「勇者を呼びますか？」の確認あり） |
| RB / LB | ゲーム速度を上げる / 下げる（x1〜x3） |
| Start | 一時停止 / 再開（一時停止画面に魔物の種類と数を表示） |
| 右スティック | カーソルを中心にカメラを回して前後左右を見る |
| RT ＋ 右スティック | カメラそのものを移動（上に倒すと崖の上の町まで見上げられる） |
| LT | カメラとカーソルを勇者の位置へ（侵攻中はそのまま追跡。カメラを動かすと解除） |
| 右スティック押し込み | ズーム切り替え（標準 → 少し遠く → さらに遠く → 標準） |
